...
readcb(struct bufferevent *bev, void *ctx)
{
	...
	if (1) {
		int srcfd = bufferevent_getfd(bev);
		int dstfd = bufferevent_getfd(partner);

		struct sockaddr_in src_addr, dst_addr;
		struct sockaddr_in src_peer, dst_peer;
		socklen_t addrlen = sizeof(src_peer);

		getpeername(srcfd, &src_peer, &addrlen);
		getpeername(dstfd, &dst_peer, &addrlen);

		getsockname(srcfd, &src_addr, &addrlen);
		getsockname(dstfd, &dst_addr, &addrlen);


		printf("================\n");
		if (srcfd == accept_fd) {
			printf("client:%d --> %d | %d --> server:%d\n",
					htons(src_peer.sin_port), htons(src_addr.sin_port),
					htons(dst_addr.sin_port), htons(dst_peer.sin_port));
		} else if (srcfd == connect_fd) {
			printf("client:%d <-- %d | %d <-- server:%d\n",
		...
		char *line;
		while ((line = evbuffer_readline(test)) != NULL) {
			printf("msg: %s\n", line);
		}
		evbuffer_free(test);
#endif
...
//	evbuffer_peek(buf, 4096, NULL, NULL, 0);
	...
	if (evbuffer_get_length(dst) >= MAX_OUTPUT) {
		printf("dst len=%d, MAX=%d\n", evbuffer_get_length(dst), MAX_OUTPUT);
    ...
    struct sockaddr *a, int slen, void *p)
{
	...
	printf("connect ok\n");
	connect_fd = bufferevent_getfd(b_out);
	printf("connect_fd = %d\n", connect_fd);

	if (ssl_ctx && use_wrapper) {
		printf("bev openssl filter\n");
